    ...
    if ($cripto != mysql_result($consulta, 0, "senha")) {
        ...
        if ($statusBase == 1) {
            ...
            for ($i = 0; $i < $data["count"]; $i++) {
                ...
                $ldapbind = ldap_bind($ldapconn, $dn, $pwd) or die("Erro na consulta da base MS-AD: " . ldap_error($ldapconn));
                echo $dn;
                if ($ldapbind) {
                    echo("Dados corretos");
                    conecta();
...
if ($admin == 2 || $admin == 3) {
    ...
    header("Location: diaria.php");
}
if ($admin == 4) {
    ?>
    <script language=javascript>
        alert('Você não pode acessar este módulo!');
        location.href = "index.php";
    </script>
    <?php
} else {
