...
// Atribuição de valores as variáveis com o recebimento dos dados dos campos do formulário solicita.php
        $getCodMarca = $_POST[setCodMarca];
        $getNomeMarca = $_POST[setMarca];
// Atualiza os dados na tabela motoristas
        $sql = "UPDATE marcas set nomeMarca='$getNomeMarca'
                         WHERE idMarca='$getCodMarca'";

        conecta();
        $sql = mysql_query($sql) or die("Houve um erro de banco de dados: " . mysql_error());
        gravaLog("Editou marca nº $getCodMarca");
        ?>

        <script language=javascript>alert('Marca de Veículo alterada com sucesso!');</script>   
